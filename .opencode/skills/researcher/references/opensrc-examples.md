...
async () => {
    ...
    const files = await opensrc.readMany(sourceName, ['package.json', 'README.md', 'src/index.ts']);
...
async () => {
    ...
    const stateHooks = await opensrc.astGrep(source.name, 'const [$STATE, $SETTER] = useState($$$INIT)', { lang: ['ts', 'tsx'], limit: 50 });
...
async () => {
    ...
    const entryPoints = allFiles.filter((f) => f.path.match(/^(src\/)?(index|main|mod)\.(ts|js)$/) || f.path.includes('/index.ts'));
...
```javascript
async () => {
    const files = await opensrc.readMany('zod', ['src/index.ts', 'src/types.ts', 'src/ZodError.ts', 'src/helpers/parseUtil.ts']);
